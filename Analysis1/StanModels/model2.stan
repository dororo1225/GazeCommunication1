...
model{
    Y ~ neg_binomial_2_log(beta0 + beta1 * X1 + beta2 * X2 + beta3 * X3 + r_pair[ID_pair], phi); 
...
generated quantities{
  ...
  for (j in 1:N){
    log_lik[j] = neg_binomial_2_log_lpmf(Y[j] | beta0 + beta1 * X1[j] + beta2 * X2[j] + beta3 * X3[j] + r_pair[ID_pair[j]], phi);
  }
}
