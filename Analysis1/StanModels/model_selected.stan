...
model{
    Y ~ neg_binomial_2_log(beta0 + beta1 * X1 + beta3 * X3 + beta4 * X4 + r_pair[ID_pair], phi); 
...
generated quantities{
  ...
  real D;                          // extremal value
  
  for (i in 1:N_predict){
    lambda[i] = exp(beta0 + beta1 * X1_predict[i] + beta3 * X3_predict[i] + beta4 * X4_predict[i] + r_pair[ID_pair_predict[i]]);
  }

  for (j in 1:N){
    log_lik[j] = neg_binomial_2_log_lpmf(Y[j] | beta0 + beta1 * X1[j] + beta3 * X3[j] + beta4 * X4[j] + r_pair[ID_pair[j]], phi);
  }
