...
model{
    Y ~ poisson_log(beta0 + beta1 * X1 + beta2 * X2 + beta3 * X3 +  beta4 * X4 + (beta5 + r_pair_slp[ID_pair]) .* X5 + beta6 * X6 + beta7 * X7 + beta8 * X8 + r_pair_int[ID_pair] + r_session[ID_session]); 
...
generated quantities{
  ...
  for (j in 1:N){
    log_lik[j] = poisson_log_lpmf(Y[j] | beta0 + beta1 * X1[j] + beta2 * X2[j] + beta3 * X3[j] + beta4 * X4[j] + (beta5 + r_pair_slp[ID_pair[j]]) * X5[j] + beta6 * X6[j] + beta7 * X7[j] + beta8 * X8[j] + r_pair_int[ID_pair[j]] + r_session[ID_session[j]]);
  }
