...
//  vector[N] X9;                                 // infant-led EC bout * (distance)^2
  ...
  int N_predict;                                // number of predictions
  real X1_predict[N_predict];                   // age in months (to obtain predictions)
  real X3_predict[N_predict];                   // average distance (to obtain predictions)
  real X4_predict[N_predict];                   // (average distance)^2 (to obtain predictions)
  int<lower=0, upper=1> X5_predict[N_predict];  // infant-led EC bout (to obtain predictions)
  real X6_predict[N_predict];                   // infant-led EC bout * age in months (to obtain predictions)
  real X8_predict[N_predict];                   // infant-led EC bout * distance (to obtain predictions)
  int<lower=1, upper=N_pair> ID_pair_predict[N_predict];              // infant-mother pair id (to obtain predictions)
}
...
model{
    Y ~ poisson_log(beta0 + beta1 * X1 + beta3 * X3 +  beta4 * X4 + (beta5 + r_pair_slp[ID_pair]) .* X5 + beta6 * X6 + beta8 * X8 + r_pair_int[ID_pair] + r_session[ID_session]); 
...
generated quantities{
  real<lower=0> lambda[N_predict]; // predicted values
  vector[N] log_lik;               //log-likelihood to calc WAIC
  real D_infant;                   // extremal value (infant-led EC bout)
  real D_mother;                   // extremal value (mother-led EC bout)
  real deltaD;                      // difference of extremal values
  real age_infant;                 // effect of age in infant-led EC bouts
  real distance_infant;            // effect of distance in infant-led EC bouts
  
  for (i in 1:N_predict){
    lambda[i] = exp(beta0 + beta1 * X1_predict[i] + beta3 * X3_predict[i] + beta4 * X4_predict[i] + (beta5 + r_pair_slp[ID_pair_predict[i]]) * X5_predict[i] + beta6 * X6_predict[i] + beta8 * X8_predict[i] + r_pair_int[ID_pair_predict[i]]);
  }

  for (j in 1:N){
    log_lik[j] = poisson_log_lpmf(Y[j] | beta0 + beta1 * X1[j] + beta3 * X3[j] + beta4 * X4[j] + (beta5 + r_pair_slp[ID_pair[j]]) * X5[j] + beta6 * X6[j] + beta8 * X8[j] + r_pair_int[ID_pair[j]] + r_session[ID_session[j]]);
  }
  
  D_infant = - (beta3 + beta8) / (2 * beta4);
  D_mother = - beta3 / (2 *beta4);
  deltaD = D_infant - D_mother;
  age_infant = beta1 + beta6;
  distance_infant = beta3 + beta8;
}
