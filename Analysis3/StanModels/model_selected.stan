...
model{
    Y ~ binomial_logit(N_bout, beta0 + beta1 * X1 + beta3 * X3 + r_pair[ID_pair] + r_session); 
...
generated quantities{
  real<lower=0, upper = 1> q[N_predict]; // predicted values
  vector[N] log_lik; //log-likelihood to calc WAIC
  
  for (i in 1:N_predict){
    q[i] = inv_logit(beta0 + beta1 * X1_predict[i] + beta3 * X3_predict[i] + r_pair[ID_pair_predict[i]]);
  }

  for (j in 1:N){
    log_lik[j] = binomial_logit_lpmf(Y[j] | N_bout[j], beta0 + beta1 * X1[j] + beta3 * X3[j] + r_pair[ID_pair[j]] + r_session[j]);
  }
}
