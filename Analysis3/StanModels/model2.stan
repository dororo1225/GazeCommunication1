...
model{
    Y ~ binomial_logit(N_bout, beta0 + beta1 * X1 + beta2 * X2 + beta3 * X3 + r_pair[ID_pair] + r_session); 
...
generated quantities{
  ...
  for (j in 1:N){
    log_lik[j] = binomial_logit_lpmf(Y[j] | N_bout[j], beta0 + beta1 * X1[j] + beta2 * X2[j] + beta3 * X3[j] + r_pair[ID_pair[j]] + r_session[j]);
  }
}
